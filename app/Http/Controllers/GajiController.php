...
class GajiController extends Controller
{
    ...
    public function index()
    {
        ...
        $tukin = DB::connection('gaji')->table('tukin')
            ...
            ->orderByDesc('tukin.tahun')
            ->orderByDesc('tukin.bulan')
            ->get();
        ...
        $um = DB::connection('gaji')->table('uang_makan')
            ->where('uang_makan.nip', $username)
            ->orderByDesc('uang_makan.tahun')
            ->orderByDesc('uang_makan.bulan')
            ->get();
    ...
    public function cetakum_ori($id)
    {
        // $username = Auth::user()->username;
    ...
    public function cetakum($id)
    {
        ...
        $filename = 'Slip_uang_makan_'.$makan->bulan.'_'.$makan->tahun."_".$makan->nama;
        // $filename = 'Slip_uang_makan_'.$makan->nama;
        // $templateProcessor->saveAs($filename . '.docx');
        // return response()->download($filename . '.docx')->deleteFileAfterSend(true);

        $wordfile = storage_path()."/slip/Slip_uang_makan_".$makan->nama.".docx";
        $templateProcessor->saveAs($wordfile);
        $tmpFile = storage_path()."/slip/".$filename.".pdf";
        $rendererName = 'DomPDF';
        $domPdfPath = base_path( 'vendor/dompdf');
