                ...
                <div class="card-header pd-t-20 pd-b-0 bd-b-0">
                    ...
                    <form class="form-inline">
                        ...
                        @if (Auth::User()->role == 1)
                            ...
                            {{-- <a href="{{ route('rename_file') }}"
                                class="btn btn-warning"
                                onclick="return confirm('Yakin ingin rename semua file?')">
                                Rename & Update File
                            </a> 

			    <a class="btn btn-success mb-2 ml-2" href="{{ route('kinerja.download', ['bulan' => $bulan, 'tahun' => $tahun]) }}"
				onclick="return confirm('Download semua file untuk bulan {{ $bulan }} - {{ $tahun }}?')">
                                Download File (.zip)
                            </a> --}}	   
                        @endif
                    </form>
                        ...
                        <table class="table mt-3" id="tabel">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ url('/kinerja/download/') }}/{{ encrypt($item->id) }}" target="_blank">{{ $item->name }}</a>
                                        {{-- <br>{{ $item->username }} --}}
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                @if ($item->id)
                                                <a class="dropdown-item" href="{{ url('/kinerja/download/') }}/{{ encrypt($item->id) }}" target="_blank">
                                                    <i data-feather="printer" class="wd-15 mg-r-5"></i>Unduh PCK</a>
...
$(document).ready(function() {
    ...
    function downloadZip() {
        ...
        var url = '/kinerja/download/' + bulan + '/' + tahun;
        window.location.href = url;
    }
</script>
@endsection
